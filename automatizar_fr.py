"""
Script independiente para automatizar documentos FR (Facturas)
Ejecutar: python automatizar_fr.py
"""
...
def automatizar_documentos_fr():
    """Automatiza todos los documentos FR (Facturas) pendientes"""
    
    print("=" * 60)
    print("AUTOMATIZACIÓN DE DOCUMENTOS FR (FACTURAS)")
    print("=" * 60)
    ...
    try:
        # Consultar facturas FR pendientes
        print("\n📋 Consultando facturas FR pendientes...")
        cursor_pg.execute("""
            SELECT 
                id, 
                nit, 
                ...
                estado_final
            FROM facturas
            WHERE clasificacion = 'Facturas' 
              AND estado_final = 'Pendiente'
            ORDER BY id
        """)
        
        facturas_fr = cursor_pg.fetchall()
        print(f"✅ Encontradas {len(facturas_fr)} facturas FR pendientes\n")
        
        if not facturas_fr:
            print("No hay facturas FR pendientes para automatizar.")
            return
        
        # Consulta SQL para FR (TIPODCTO hardcodeado como 'FR')
        query_fr = """
            SELECT 
                NRODCTO, 
                PASSWORDIN, 
            ...
            FROM TRADE
            WHERE LTRIM(RTRIM(dctoprv)) = ? AND NIT = ? AND TIPODCTO = 'FR' AND ORIGEN = 'COM'
        """
        ...
        archivo_respaldo = f"ids_actualizados_fr_{timestamp}.txt"
        ids_actualizados = []
        
        print(f"📝 Archivo de respaldo: {archivo_respaldo}\n")
        
        for factura in facturas_fr:
            ...
            try:
                # Buscar en SQL Server (solo dctoprv y NIT, TIPODCTO está hardcodeado como 'FR')
                cursor_sql.execute(query_fr, (numero_factura, nit))
                ...
                if resultados:
                    ...
                    print(f"      Parámetros usados: dctoprv='{numero_factura}', NIT='{nit}', TIPODCTO='FR'")
        ...
        if ids_actualizados:
            try:
                with open(archivo_respaldo, 'w', encoding='utf-8') as f:
                    f.write(f"# Respaldo de automatización FR - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        print(f"📊 Total procesadas: {len(facturas_fr)}")
        if ids_actualizados:
            print(f"💾 Archivo de respaldo: {archivo_respaldo}")
        print("=" * 60)
